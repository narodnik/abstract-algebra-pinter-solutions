...
$\vec c$ is uniquely written as $\vec c = \vec a + \vec b$ means that $\vec c = k \bar{\vec a} + l \bar{\vec b}$ where $\bar{\vec a}$ and $\bar{\vec b}$ are the basis of $T$ and $U$. These basis are thus independent and $k\bar{\vec a} + l\bar{\vec b} = \vec 0 \implies k = l = 0$. Thus there is no element both in $T$ and $U$ otherwise there would be nonzero $k, l$ such that $k\bar{\vec a} = -l\bar{\vec b}$ which is impossible.

## Q3

$T$ has a basis of length $k$
$$\phi(a_1 + \cdots + a_k + a_{k + 1} + \cdots + a_n) = a_1 + \cdots + a_k$$
$$\pi(a_1 + \cdots + a_k + a_{k + 1} + \cdots + a_n) = a_{k + 1} + \cdots + a_n$$
These are both homomorphisms.
$$T = \ran \phi, U = \ran \pi$$
$$V = T + U$$
$$T \cap U = \{ \vec 0 \}$$
